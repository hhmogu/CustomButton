...
function smarty_function_sugar_button($params, &$smarty)
{
    ...
    if (!is_array($type)) {
        ...
        if(isset($type['btn_name'])){
            //权限查询和条件判断
            $bean = BeanFactory::getBean($params['module']);            
            $has_access = ACLController::checkAccess($bean->module_dir, $type['btn_name'], 'not_set', $bean->acltype, 'not_set');
            $GLOBALS['log']->fatal('show_button=='.$has_access);
